...
const generatePrompt = (driverName, riskEvents) => {
  ...
  return `
    You are a fleet safety analyst. Given ${driverName} ‘s risk events (listed below), identify the top two most prevalent patterns in their risky driving. Format your response in two sentences such as: 'Name Here tends to exhibit [risk behavior] after [time period/event]' and 'Name Here also frequently [another risk behavior] when [time period/event]'. This information will be used to improve driver safety. These risk events are from a system which provides fleet managers with insights into how their drivers are driving. The response should include some information about their tendencies and behaviors. Some ideas for patterns include time of day, which events usually occur at said time of day, certain events commonly following other events, and any other tendencies that you can detect. If you decide to mention a time in your response, use 12 hour time and dont be specific down to the minute, just mention general times like “rush hour”, “morning hours”, and “late night”. Times are given in UTC but you should convert them to EST if you optionally decide to mention them. Your response should be brief and be able to quickly tell the reader the tendencies of this driver. Respond only with the short 2 sentence summary, and nothing else.
